...
export const titleCSS = 'text-2xl sm:text-3xl uppercase font-bold text-red mt-2 sm:mt-12' + ' mb-4 font-sifonn';
...
const getFestivalDate = (festival: IFestival) =>
  getDatesBetween(new Date(festival.startDate), new Date(festival.endDate), true);
...
const Programmation: React.FC = () => {
  ...
  return (
    <div className="flex flex-col z-10 page-content">
      ...
      {festival.id && festival.showMusic ? (
        getFestivalDate(festival).map((date, index) => (
          <div key={index}>
            <p className={dateCSS}>{formatDate(date)}</p>
            <div className="grid grid-cols-2 gap-2 sm:gap-10">
              {musics
                .filter((music) => datesMatched(music.publicationDate, date))
                .map((music, key) => (
      ...
      {festival.id && festival.showMovie ? (
        getFestivalDate(festival).map((date, index) => (
          <div key={index}>
            <p className={dateCSS}>{formatDate(date)}</p>
            <div className="grid grid-cols-2 gap-2 sm:gap-10">
              {movies
                .filter((film) => datesMatched(film.publicationDate, date))
                .map((film, key) => (
