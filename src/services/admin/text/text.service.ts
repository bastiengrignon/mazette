...
export class TextService {
  ...
  static getByTextType = async (type: TextType): Promise<IText> =>
    await axiosInstance.get(`/textType/${type}`).then((r) => r.data);

  static create = async (text: IText): Promise<IText> => await axiosInstance.post('/text', text).then((r) => r.data);

  static update = async (id: number, updatedText: IText): Promise<IText> =>
    await axiosInstance.put(`/text/${id}`, updatedText).then((r) => r.data);
