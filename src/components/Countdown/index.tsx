...
const Countdown: React.FC<CountdownProps> = ({ deadline, text, invertColor }) => {
  ...
  const leadingZero = (number: number): string | number => (number < 10 ? '0' + number : number);
  ...
  return (
    <div
      className={`w-full ${invertColor ? 'text-white' : 'text-white'} text-lg sm:text-2xl md:text-4xl lg:text-6xl p-2 ${
        invertColor ? 'bg-green' : 'bg-red'
      }`}>
      <div className="flex justify-center space-x-1 sm:space-x-5 md:space-x-10">
        <div className={countdownNumberCSS}>
          {leadingZero(days)}
      ...
      <div className={'flex justify-center italic text-xl sm:text-4xl md:text-5xl mt-4 text-white'}>{text}</div>
    </div>
  );
};
