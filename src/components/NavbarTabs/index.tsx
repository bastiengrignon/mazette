...
const popoverItemClass =
  'p-2 flex justify-end rounded-none bg-red text-right uppercase text-white text-base lg:text-xl no-underline hover:bg-white hover:text-red';
...
const NavbarTabs: React.FC<NavbarTabsProps> = ({ className, style, isMenuOpen, setMenuOpen, isMobile = false }) => {
  ...
  return (
    <div className={`space-x-0 lg:space-x-5 space-y-5 lg:space-y-0 mr-5 uppercase ${className}`} style={style}>
      ...
      <NavLink
        to={RouterUrl.information}
        onClick={() => setMenuOpen(false)}
        className={({ isActive }) =>
          `${tabClass} ${isMenuOpen ? 'block' : 'hidden'} ${isActive ? (isInformationURL ? activeClass : '') : ''}`
        }>
        {TabName.information}
      </NavLink>
    </div>
