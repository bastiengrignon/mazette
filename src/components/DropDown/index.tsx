...
const DropDown: React.FC<DropDownProps> = ({name, className, items, onItemClick}) => {
    ...
    return (
        <div className={`relative ${className} focus:outline-none cursor-pointer`}
            ...
            <div
                className={`${open ? "flex" : "hidden"} flex-col text-right bg-test-red text-white lg:absolute lg:right-0 last:rounded-b-md`}>
                {
                    items.map((item, index) => (
                        <NavHashLink key={index} to={`${item.link}#${item.name}`} role="menuitem"
                            onClick={handleClick} isActive={isActive}
                            className="w-full pr-1 py-2 hover:bg-white hover:text-test-red border border-transparent hover:border-test-red font-light text-sm md:text-base lg:text-xl lg:last:rounded-b-md lg:pl-5 lg:py-1">
                            {item.name}
                        </NavHashLink>
                    ))
                }
